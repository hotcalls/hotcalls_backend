"""
Baseline health checks for hotcalls django application.
"""
...
def readiness_check(request):
    ...
    Checks database connectivity, cache availability, and redis connectivity.
    """
    checks = {
        "database": False,
        "cache": False,
    ...
    try:
        ...
        logger.info(
            f"Database connection attempt - Host: {db_settings.get('HOST')}, "
...
def startup_check(request):
    """
    Basic Startup check endpoint.

    Tries to load secret key setting to see if application started.
    """
    try:
